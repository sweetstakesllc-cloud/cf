...
export function registerProfitRoutes(app:FastifyInstance,pool:pg.Pool,shopify:ShopifyAdminClient,config:Config){
 ...
 app.register(async protectedApp=>{
  ...
  protectedApp.get('/profit/api/report',async(request,reply)=>{
   ...
   for(const row of included){const p=periods.get(row.day)||{day:row.day,orders:0,netSales:0,cogs:0,fees:0,result:0,missing:0};p.orders++;p.netSales+=row.netSales;p.cogs+=row.cogs;p.fees+=row.fees;p.result+=row.knownCostsResult;if(!row.complete)p.missing++;periods.set(row.day,p);}
   const totals={orders:included.length,netSales:0,cogs:0,fees:0,shipping:0,packaging:0,result:0,refunds:0,expenses:0,incomplete:0,missingCost:0,missingFees:0,missingShipping:0,missingPackaging:0};
   for(const row of included){for(const key of ['netSales','cogs','fees','result','refunds'] as const)totals[key]+=key==='result'?row.knownCostsResult:row[key];totals.shipping+=row.shipping||0;totals.packaging+=row.packaging||0;if(!row.complete)totals.incomplete++;totals.missingCost+=row.missingCost;if(row.missingFees)totals.missingFees++;if(row.shipping==null)totals.missingShipping++;if(row.packaging==null)totals.missingPackaging++;}
   for(const expense of expenses.rows){const day=expense.day;const p=periods.get(day)||{day,orders:0,netSales:0,cogs:0,fees:0,result:0,missing:0};p.result-=Number(expense.amount);periods.set(day,p);}
   totals.expenses=expenses.rows.reduce((sum,r)=>sum+Number(r.amount),0);totals.result-=totals.expenses;
